...
The current list has 177 words: 27 three-character, 80 four-character, and 70 five-character entries. After the 12–16 character length rule is applied, it yields **3,963,960 valid ordered word triples**. With 100 digit pairs, 3 digit positions and 4 symbols, that is **4,756,752,000 combinations (~32.1 bits)**. About 72.7% of distinct ordered word triples fit the length budget. Any change to the word list, digit positions, symbol set, or length budget changes these numbers — recompute them rather than assuming.

Earlier revisions ran at ~34.1 bits (312 words, 8 symbols); trimming to phone-friendly words and dropping four symbols cost roughly 4 bits, a deliberate usability trade. Removing American-only and animal words reduced the pool further, while varying the digit position recovered about 1.6 bits without making passwords harder to dictate. The August 2026 phone-readability pass expanded the curated pool from 127 to 177 words while removing exact homophones and risky near-homophones, raising simple-mode entropy from ~30.6 to ~32.1 bits. If entropy needs to come back up further, add more clear 3–5 character words rather than restoring ambiguous symbols or poor phone words.
