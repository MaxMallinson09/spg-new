...
- Format: three words, with two adjacent digits placed after the first, second, or third word, then one special character (e.g. `Jam95OnionDesk*` or `JamOnion95Desk*`).
...
- **Every word must be 3–5 characters.** This is a correctness constraint, not style: three words plus two digits and a symbol have to fit the 12–16 character budget, and over-long candidates are discarded by `generatePassword()`. Adding longer words shrinks the usable keyspace instead of growing it.
...
The current list has 147 words: 20 three-character, 65 four-character, and 62 five-character entries. After the 12–16 character length rule is applied, it yields **2,147,580 valid ordered word triples**. With 100 digit pairs, 3 digit positions and 4 symbols, that is **2,577,096,000 combinations (~31.3 bits)**. About 69.0% of distinct ordered word triples fit the length budget. Any change to the word list, digit positions, symbol set, or length budget changes these numbers — recompute them rather than assuming.

Earlier revisions ran at ~34.1 bits (312 words, 8 symbols); trimming to phone-friendly words and dropping four symbols cost roughly 4 bits, a deliberate usability trade. Removing American-only and animal words reduced the pool further, while varying the digit position recovered about 1.6 bits without making passwords harder to dictate. The August 2026 phone-readability pass expanded the curated pool from 127 to 147 words after user review, while removing exact homophones and risky near-homophones, raising simple-mode entropy from ~30.6 to ~31.3 bits. If entropy needs to come back up further, add more clear 3–5 character words rather than restoring ambiguous symbols or poor phone words.
