...
Click "New password" to generate a fresh password in the format `WordWordWordNN!` (for example `MangoDesk27Plum!`). Words come from a curated 146-word list of neutral, familiar terms chosen to be easy to hear and write down over the phone. The list deliberately avoids animal names, exact homophones, silent-letter spellings, obscure words, awkward or offensive terms, and words that can sound embarrassing on a poor line.

Randomness comes from the browser's cryptographic RNG (`crypto.getRandomValues`). The current simple-mode keyspace contains 2,542,608,000 valid combinations (about 31.2 bits) after the 12–16 character length rule is applied.
